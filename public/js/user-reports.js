document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- React for comments ---
  async function reactThumb(commentId, type, upEl, downEl) {
    try {
      const res = await fetch(`/api/comment-reactions/${commentId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type })
      });
      ...
      upEl.querySelector(".count").textContent = data.thumbs_up;
      downEl.querySelector(".count").textContent = data.thumbs_down;

      upEl.classList.toggle("reacted", type === "up");
      downEl.classList.toggle("reacted", type === "down");
    } catch (err) {
      console.error(err); alert("Tatizo kupiga thumb");
    }
  }
    ...
    li.innerHTML = `
      ...
      <div>
        ...
        <div class="comment-thumbs">
          <span class="thumb-up">👍 <span class="count">${c.thumbs_up || 0}</span></span>
          <span class="thumb-down">👎 <span class="count">${c.thumbs_down || 0}</span></span>
        </div>
      </div>
    `;
    ul.prepend(li);

    const thumbsUp = li.querySelector(".thumb-up");
    const thumbsDown = li.querySelector(".thumb-down");

    thumbsUp.addEventListener('click', async () => reactThumb(c.id, 'up', thumbsUp, thumbsDown));
    thumbsDown.addEventListener('click', async () => reactThumb(c.id, 'down', thumbsUp, thumbsDown));
  }

  // Load existing comments
  r.comments.forEach(c => renderComment(c));
  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      ...
      renderComment(newComment); // ✅ thumbs included
